...
(function () {
  ...
  var STATUS = { progress: "進行中", active: "契約中", canceled: "終了" };
    ...
    api.getOrder(orderId).then(function (o) {
      ...
      api.getThread(orderId).then(function (msgs) {
        ...
        document.getElementById("room-form").addEventListener("submit", function (e) {
          ...
          if (!text) return;
          api.sendMessage(orderId, text).then(function (all) {
  ...
  function bubble(m) {
    ...
    return (
      ...
      '<div class="msg__bubble">' + esc(m.body) + "</div>" +
      '<p class="msg__time">' + esc(m.timeLabel) + "</p></div>"
    );
  }
})();
