...
(function () {
  ...
  function wireOnbSlots() {
    ...
    var wk = document.getElementById("o-slot-week");
    if (wk) wk.addEventListener("click", function () {
      var inp = document.getElementById("o-slot-input");
      var v = inp.value;
      if (!v) return UI.toast("日時を選んでください");
      for (var i = 0; i < 4; i++) { var nv = App.addWeeks(v, i); if (draft.slots.indexOf(nv) === -1) draft.slots.push(nv); }
      draft.slots.sort(); renderOnbSlotChips(); inp.value = "";
    });
  }
